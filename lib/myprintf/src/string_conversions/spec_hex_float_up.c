...
int hex_float_to_str_up(int precision, double copy, char *result, int len)
{
    int hex_dig = 0;
...
double sign_implementer(double num, double copy, char *result, int *len)
{
    int sign = (num < 0) ? -1 : 1;
...
char *double_to_hex_up(double num, char *result, int precision)
{
    double copy = num;
    int exponent = 0;
    int len = 0;

    copy = sign_implementer(num, copy, result, &len);
    while (copy >= 2.0) {
        copy /= 2.0;
        exponent++;
    }
    ...
    len = len + 4;
    len = hex_float_to_str_up(precision, copy, result, len);
    exponent = suffix(len, result, exponent);
    result = strcat_and_numstr(exponent, result, len);
    return result;
}

char *specifier_float_to_hex_up(double num, int precision)
{
    char *result = malloc(50 * sizeof(char));

    result = double_to_hex_up(num, result, precision);
    return result;
}
